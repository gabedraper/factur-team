/*
 * Orum's payload as it actually arrives, and the calls it makes to lists.
 *
 * The first real disposition (2026-09-18, Lance Rhodes, Left Voicemail)
 * showed the call one level deeper than guessed -- payload.v1 -- under
 * names none of the guesses covered, so the resolver read a call with no
 * caller, no number and no time, and the route keyed every event on a
 * digest of nothing, so each one overwrote the last. The names are read
 * now, and two are better than anything hoped for: prospectFID is the
 * Salesforce opportunity the list row came from, which the resolver takes
 * first; recordingLink carries the recording id Salesforce keeps on its
 * Task, so the two paths meet by key. The dial mode is in the note.
 *
 * Then the finding underneath. 3,337 of the ~3,370 opportunities Orum
 * dialled this week are "Prospecting: Cold Call List", which the app leaves
 * in Salesforce on purpose: a list row is not a pursuit of anyone yet.
 * Those calls have no home here, by either path. So the resolver says so
 * ("cold call list: not a pursuit in the app yet") rather than "no contact
 * matches", and a nightly job puts such events back in front of it once
 * their opportunity has arrived in the app -- the dials that led to a lead
 * then appear on its timeline the day it becomes one.
 */
...
/* Cold-list calls come back for another look once their opportunity is a
   pursuit. Cheap: only events whose named opportunity now exists here. */
create or replace function public.retry_cold_list_events()
returns integer
language plpgsql
security definer
set search_path to 'public'
...
begin
  update public.activity_events e
   ...
   where e.status = 'needs_review'
     and (e.resolution ->> 'cold_list')::boolean
     and e.received_at > now() - interval '60 days'
     and exists (
       select 1 from public.opportunities o
        where o.salesforce_opportunity_id = e.resolution -> 'facts' ->> 'sf_opportunity_id'
     );
...
revoke all on function public.retry_cold_list_events() from public, anon;
grant execute on function public.retry_cold_list_events() to service_role;

select cron.schedule(
  'retry-cold-list-events',
  '50 3 * * *',
  $cron$ select public.retry_cold_list_events(); $cron$
);

/* What Orum sent under the old reading goes past the resolver again. */
update public.activity_events
   set status = 'pending', attempts = 0
 where source = 'orum' and status in ('needs_review', 'failed', 'resolved');
