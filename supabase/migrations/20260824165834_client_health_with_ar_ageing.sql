 ...
 * The number of contributing inputs travels with the score, so one resting on a
 * single signal can be told apart from one resting on five.
 *
 * Receivables come from the QuickBooks ageing report rather than the single
 * balance figure on the Salesforce record -- the books are where the money
 * actually lives, and being owed £10k for a fortnight is a different thing from
 * being owed £10k since spring.
 */
...
as $$
  ...
  leads as (
    select b.id,
           ...
           count(l.id) as total, count(l.contact_title__c) as with_title
    from base b
  ...
  scored as (
    select b.*, l.recent leads_recent, l.prior leads_prior, l.quoted, l.no_quoted,
           ...
           -- How much of what they owe has gone stale. Everything current is
           -- 100; everything past ninety days is 0; the buckets in between
           -- count against it by how late they are.
    ...
    left join client_ar ar on ar.client_id = b.id
  ),
  parts as (
  ...
  select s.id, s.name, s.status, s.am, s.manual,
         ...
         s.engagement_score,
         s.ar_total, s.overdue_60_plus, s.receivables_score,
