/*
 * The board lists every client with a QuickBooks balance, current or former.
 *
 * The client set no longer filters on active, the QuickBooks match is asked the
 * wider question, and each row carries client_active so a former client can be
 * marked as one rather than sitting there looking like a live account.
 */
...
as $function$
  ...
  /*
   * Former clients included. A debt does not stop being a debt when the client
   * stops being a client, and seventeen of them owe six figures between them.
   */
  scoped as (
  ...
  select late.client_id, c.name,
         (c.active and coalesce(c.status, '') <> 'Inactive') as client_active,
         late.qb_customer_name,
