    ...
    <div>
        ...
        {weights.map((w) => (
          <div key={w.effort_source} className="flex items-center gap-4 py-3">
            <div className="flex-1">
              <p className="text-body">{w.effort_source}</p>
              {w.description && (
                <p className="text-meta text-neutral-500">{w.description}</p>
              )}
            </div>
            <input
              type="number"
              ...
              className={control({ size: "sm", className: "w-20 text-right [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none" })}
            />
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 flex items-center gap-3">
        <button
          ...
          className="rounded-md bg-primary px-4 py-2 text-body font-medium text-primary-foreground disabled:cursor-not-allowed disabled:opacity-50"
        >
          {isPending ? "Saving…" : "Save"}
        </button>
        {saved && <span className="text-body text-emerald-400">Saved</span>}
      </div>
    </div>
  );
}
