...
export default async function QuotesDataPage({ searchParams }: { searchParams: Promise<Search> }) {
  await requirePipeline("view");
  const sp = await searchParams;
  const page = Math.max(0, Number(sp.page ?? 0) || 0);

  const [{ rows, hasMore }, statuses, client] = await Promise.all([
    browseCommerce({ kind: "quotes", q: sp.q, clientId: sp.client ?? null, status: sp.status ?? null, page }),
