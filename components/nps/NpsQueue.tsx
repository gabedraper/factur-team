...
export function NpsQueue({
  ...
  return (
    <div className="space-y-6">
      ...
      {stepsActive === 0 && (
        <p className="rounded-md border border-amber-400 bg-amber-50 px-3 py-2 text-body text-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
      ...
      <div className="space-y-2">
          ...
          return (
            <Surface key={key(r)} pad="none">
              <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 px-4 py-3">
                <span className="rounded-md border px-1.5 py-0.5 text-meta tabular-nums">
                  Step {r.step_position}
                </span>
                <span className="font-medium">{r.client_name}</span>
                <span className="text-body text-muted-foreground">{r.to_email}</span>
        ...
        {rows.length === 0 && stepsActive > 0 && (
          <p className="text-body text-muted-foreground">Nothing due.</p>
        )}
      </div>
    </div>
  );
