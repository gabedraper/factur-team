export function TeamBlurb({
  ...
  return (
    <div className="pointer-events-none absolute right-full top-1/2 z-10 -translate-y-1/2 pr-3 group-hover:pointer-events-auto">
      <div className="relative max-h-96 w-72 overflow-y-auto rounded-md border border-slate-800 bg-slate-900 p-3 text-xs opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
        ...
        {team.map((t, idx) => (
          <div
            key={idx}
            className="flex items-center justify-between gap-2 border-t border-slate-800 py-1.5 first:border-t-0"
          >
