...
import {
  OPERATOR_LABELS, operatorsFor, opTakesNoValue,
  type BaseField, type Filter, type ListView, type Operator,
 ...
 * people moving off Salesforce will look for it.
 *
 * Entity-agnostic on purpose. It is handed a field catalogue and two functions
 * that write, so the clients table and anything after it share one editor
 * rather than growing a second copy that slowly disagrees about what a filter
 * row looks like. The opportunities list still has its own; this is where it
 * should land when someone next touches it.
 *
...
export type SaveResult = { success: boolean; id?: string; error?: string };

export type ViewEditorSetup = {
  /** Every column and filter this list may offer. */
  fields: BaseField[];
  /** Values for each named picklist, usually taken from the rows on screen. */
  picklists?: Record<string, string[]>;
  defaultColumns: string[];
  /** Shown where the filter list is empty: "Every client you can see." */
  unfilteredNote: string;
  canShare: boolean;
  save: (input: {
    id?: string;
    name: string;
  ...
  /** Where deleting a view leaves you. */
  afterDeleteHref: string;
};

function blank(setup: ViewEditorSetup): ListView {
  return {
    id: "", name: "", owner_member_id: null, shared: false,
    columns: [...setup.defaultColumns], filters: [], sort_field: null, sort_dir: "asc",
  };
}

/** The Edit / New buttons that sit beside the view chips. */
export function ViewTools({
  ...
  return (
    <>
      ...
      <Button variant="ghost" size="sm" onClick={() => setEditing(blank(setup))}>
        <Plus className="mr-1 h-3.5 w-3.5" /> New view
      </Button>
      {editing && (
        <ViewEditor view={editing} setup={setup} onClose={() => setEditing(null)} />
      )}
    </>
  );
}
...
export function ViewEditor({
  ...
  const save = () => {
    setError(null);
    start(async () => {
      const res = await setup.save({
        id: view.id || undefined, name, shared, columns, filters, sortField, sortDir,
      });
  ...
  const remove = () => {
    setError(null);
    start(async () => {
      const res = await setup.remove(view.id);
      if (!res.success) { setError(res.error ?? "Could not delete the view."); return; }
      onClose();
      router.push(setup.afterDeleteHref);
      router.refresh();
    });
  };
